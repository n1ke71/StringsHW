...
//  StringsHW
//
//  Created by Ivan Kozaderov on 27.01.2018.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSString* testString = [text substringToIndex:208];
    
    NSLog(@"testString\n%@",testString);
    
    NSLog(@"first half %@\n last half%@\n",[testString substringToIndex:[testString length]/2],[testString substringFromIndex:[testString length]/2] );
   ...
   for (NSString*string in array1) {
    ...
    NSString* firstWord = [tempArray firstObject];
    NSString* lastWord  = [tempArray lastObject];
    NSString* resultString;
         
    NSLog(@"starting string \n %@",string1);
         
         firstWordRange = [string1 rangeOfString: firstWord];
         
         if(firstWordRange.location !=NSNotFound){
         
         resultString = [string1 substringFromIndex:firstWordRange.length];
         
         NSLog(@"resultString after cuted first word \n %@",resultString);
         }
         
         
         lastWordRange  = [resultString rangeOfString: lastWord];
         
         if(lastWordRange.location !=NSNotFound){
         
         resultString = [resultString substringToIndex:lastWordRange.location];
             
         NSLog(@"resultString after cuted last word \n %@",resultString);
         }
         
         resultString = [NSString stringWithFormat:@"%@ %@ %@",lastWord,resultString,firstWord];
    ...
    NSString* superText = text;
    
    NSArray* sentences = [superText componentsSeparatedByString:@"."];
    
    NSMutableArray* resultSentences = [NSMutableArray array];
    ...
    for (NSString* word in sentences ) {
        ...
        NSString* resultString = [NSString stringWithFormat:@"%@.",[self caapitalizedLeter:lowerCaseString]];
        
       // NSLog(@"resultString:%@",resultString);
        
        [resultSentences addObject:resultString];
        }
    }
    
    NSString* done = [resultSentences componentsJoinedByString:@""];
...
-(NSString*)caapitalizedLeter:(NSString*) string{
    
    NSString* new = @"";
    NSString* pattern = @"([a-z]\\b)"; //Last letter in every word
    NSError*  error = NULL;
    ...
    for (NSString* string in array) {
        ...
        if (match) {
            ...
            NSString* letter = [[string substringWithRange:matchRange] uppercaseString];
            
        //    NSLog(@"string%@",letter);
            
            NSString* part   = [string substringToIndex:[string length] - 1];
            
       //     NSLog(@"part%@",part);
            
            NSString* result = [NSString stringWithFormat:@"%@%@",part,letter];
            
        //    NSLog(@"result%@",result);
            
            new = [NSString stringWithFormat:@"%@ %@",new,result];
